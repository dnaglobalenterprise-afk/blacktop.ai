...
var PayoutsService = function () {
    ...
    var PayoutsService = _classThis = /** @class */ (function () {
        function PayoutsService_1(prisma) {
            this.prisma = prisma;
        }
        /**
         * ✅ Calculate Payout based on Percentage of Gross
         */
        PayoutsService_1.prototype.calculatePercentagePayout = function (gross, percentage) {
            var driverPay = gross * (percentage / 100);
            return {
                type: 'PERCENTAGE',
                gross: gross,
                rate: "".concat(percentage, "%"),
         ...
         */
        PayoutsService_1.prototype.calculateMileagePayout = function (loadedMiles, loadedRate, deadheadMiles, deadheadRate) {
            var loadedPay = loadedMiles * loadedRate;
            var deadheadPay = deadheadMiles * deadheadRate;
            var totalPay = loadedPay + deadheadPay;
            return {
                type: 'MILEAGE',
                ...
                breakdown: {
                    loaded: "".concat(loadedMiles, " @ $").concat(loadedRate),
                    deadhead: "".concat(deadheadMiles, " @ $").concat(deadheadRate)
                }
            };
        };
